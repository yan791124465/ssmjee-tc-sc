                    ...
                    <tr>
                        ...
                        <td>
                            <div class="btn-group btn-group-xs btn-group-solid ">
                                ...
                                <button data-id="${item.id}" class="btn btn-warning" onclick="delPower(this)">授权</button>
                            </div>
                        </td>
                    </tr>
                </c:forEach>
...
<script>
     ...
     * @param obj 对象
     */
    function delPower(obj) {
        var roleId = $(obj).data("id");
        $.post('/power/findByRoleInfo', {roleId: roleId}, function (html) {
            window.layer_roleEdit = layer.open({
                title: "角色授权",
                id: "roleEdit",
                type: 1,
                area: ['400px', '600px'],
                ...
                yes: function (index, layero) {
                    //更新角色信息
                    var powers = getAllCheckedPower();
                    tools.post("/role/changePower", {roleId: roleId, powers: powers.join(",")}, function (tip) {
                        if (tip.success) {
                            layer.msg('修改成功', {icon: 1, time: 1000});
                            layer.close(layer_roleEdit);
                        } else {
     ...
     */
    function addModule(){
        tools.loadPage("/role/add", null, function(html){
    ...
    function editModule(id){
        $.post("/role/edit", {"id":id}, function(html){
    ...
    function deleteById(id){
        layer.confirm("确定删除么？",function() {
            tools.post("/role/delete", {"id":id}, function (data) {
                if(data.success){
    ...
    function deleteByIds(){
        ...
        layer.confirm("确定删除选中信息么？",function(){
            tools.post("/role/deleteByIds",{"ids":ids.join(",")},function(data){
                if(data.success){
